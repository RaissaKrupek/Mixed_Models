...
rm(list=ls()) 
setwd("C:/Users/sgeza/OneDrive/Desktop/Workshops ASReml-R/ASReml_ESALQ/Material/Session10")
library(asreml)
library(ASRgenomics)
...
modelGBLUPf <- asreml(fixed = WT_HB~1,
                    random = ~vm(INDIV, Ginv.blend) + Family,
                    residual = ~idv(units),
                    workspace = 128e06,
                    na.action = na.method(y = "include"),
                    data = datag)
...
#######################################
# Linking to get betas

BLUP <- as.data.frame(summary(modelGBLUP, coef=TRUE)$coef.random)
head(BLUP) # GEBVs

# a = M*beta
# M'*a = M'*M*beta / inv(M'*M)
# beta =  inv(M'*M) * M' * a
#  p*1  =     p*p          p*n    n*1  
...
beta0 <- mean(datag$WT_HB, na.rm=TRUE)
yhatGP <- beta0 + M %*% beta
head(yhatGP)
